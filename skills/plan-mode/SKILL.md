...
Write your plan to the issue `plan` document using `PUT /api/issues/{issueId}/documents/plan`. Build this document incrementally as you progress through the steps below — do not wait until the end to write it all at once. If a plan document already exists, fetch it first via `GET /api/issues/{issueId}/documents/plan` and include its `baseRevisionId` when updating. Decide whether the current request is a new task (overwrite) or a continuation (revise).
...
Resolve ambiguities before committing to an approach. This may cover technical implementation, desired system behavior, UI/UX, performance, edge cases, ownership of state, failure handling, or tradeoffs.

- If you need human input, post a comment on the issue with your clarifying questions.
- If a design decision needs approval, use a `request_confirmation` interaction tied to the latest plan revision.
- If an architectural question belongs to another agent (e.g., the Architect owns API contract decisions, Leads own implementation detail), escalate via issue comment mentioning the appropriate agent.
- You may ask multiple rounds, reading more code in between. Do not make large assumptions about intent.

## Step 3: Design

Based on exploration and clarification, design a concrete implementation approach:
...
2. Confirm the approach aligns with the original intent, not just a plausible interpretation of it.
...
5. Resolve remaining ambiguities via issue comments or `request_confirmation` interactions.

## Step 5: Present the Plan

Write the final plan to the issue `plan` document. The plan should be:
...
After writing the plan, open a `request_confirmation` interaction tied to the latest plan revision and set the issue to `in_review`. Wait for approval before proceeding.

If feedback is received, revise the plan document and open a new confirmation tied to the updated revision.

## Step 6: Hand Off

**Plan-mode agents do not implement.** Once the plan is approved:

- Reassign the issue to the appropriate build-mode agent (e.g., Frontend Lead, Backend Lead, or a Dev agent).
- If the work must be split across multiple agents, create subtask issues with `parentId`, `goalId`, and `blockedByIssueIds` to express dependencies.
- Use the `paperclip-converting-plans-to-tasks` skill when breaking down complex plans into executable Paperclip issues.

**Build-mode agents** who receive an approved plan follow Steps 1–4 for understanding, then execute the plan directly in Step 6 instead of handing off.